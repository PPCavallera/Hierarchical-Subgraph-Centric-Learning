from lib.get_data_windmill_lstm import get_prepared_dataloader as get_prepared_dataloader_lstm, get_precise_cluster as get_precise_cluster_lstm, get_raw_data as get_raw_data_lstm, get_prepared_dataset as get_prepared_dataset_lstm
from lib.get_data_windmill_dcrnn import get_prepared_dataloader as get_prepared_dataloader_dcrnn, get_raw_data as get_raw_data_dcrnn, get_precise_cluster  as get_precise_cluster_dcrnn, get_processed_data as get_processed_data_dcrnn
...
for i in range(len(Y_true[0])):
    ...
    ax.set_title('WINDMILLL - Model comparison for node ' + str(i+1), fontsize=14)
    ax.set_xlabel('X', fontsize=12)
    ax.set_ylabel('Value', fontsize=12)
    ax.grid(True, linestyle='--', alpha=0.5)
    ax.legend(loc='upper left', bbox_to_anchor=(1, 1), fontsize=10)
    plt.tight_layout()
    plt.savefig('WINDMILLL_node'+str(i+1)+'.png')