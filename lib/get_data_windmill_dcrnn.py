...
def get_processed_data(res, nb_past, cluster_index):
    with open("./data/windmill_large.json") as f:
        data = json.load(f)
    targets = []
    for time in range(data["time_periods"]):
        targets.append(np.array(data["block"][time]))
    ...
    edges = (np.asarray(data["edges"])).reshape((2, len(data['edges'])))
    edges_weights = np.asarray(data['weights'])
    if res is not None:
        li1 = []
        li2 = []
        li_weights = []
