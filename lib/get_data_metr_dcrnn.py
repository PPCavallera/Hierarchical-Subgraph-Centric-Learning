...
def get_raw_data():
    ...
    df =pd.read_csv('./data/metr-la.csv', index_col="Unnamed: 0")

    with open("./data/adj_mx.pkl", 'rb') as file:
        pkl_data = pickle.load(file, encoding='latin1')
    
    
    A = np.array(pkl_data[2])
    df.rename(columns=pkl_data[1], inplace=True)
    scaler = MinMaxScaler()
    ...
    for i in range(A.shape[0]):
        for j in range(A.shape[1]):
            if A[i][j] != 0:
                ...
                li2.append(j)
                li_w.append(A[i][j])
    return np.stack([li1, li2]), np.asarray(li_w)
...
def get_processed_data(res, nb_past, cluster_index):
    ...
    df = df.transpose()
    stacked_target = np.stack(df.T.values)
    X = np.array([
    ...
    with open("./data/adj_mx.pkl", 'rb') as file:
        pkl_data = pickle.load(file, encoding='latin1')
    edges, edges_weights = create_edges_list(np.array(pkl_data[2]))
    if res is not None:
        li1 = []
        li2 = []
        li_weights = []
